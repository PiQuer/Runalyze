...
class SectionCompositeRow extends TrainingViewSectionRowTabbedPlot {
	...
	protected function setRightContent() {
		...
		if ($this->Context->trackdata()->has(\Runalyze\Model\Trackdata\Entity::PACE)) {
			...
			$Code = $Table->getCode();
			$Code .= HTML::info( '<a class="window" href="settings/sport/'.$this->Context->activity()->sportid().'/zone/pace">'.__('Configure your zones').'</a>' );
		...
		if ($this->Context->trackdata()->has(\Runalyze\Model\Trackdata\Entity::HEARTRATE)) {
			...
			$Code = $Table->getCode();
            $Code .= HTML::info( '<a class="window" href="settings/sport/'.$this->Context->activity()->sportid().'/zone/heartrate">'.__('Configure your zones').'</a>' );
